...
public class VRSettingsHelper {
    ...
    private static float calculateUndistortionFactor(float r2,Distortion distortion){
        final int size=10;
        ...
        for(int i=0;i<size;i++){
            for(int j=0;j<size;j++){
                ...
                final float _r2=(float)Math.sqrt(x*x+y*y);
                final float factor=distortion.distort(_r2)-r2;
                final float diff=Math.abs(factor-r2);
                if(diff<bestDiff){
                    bestDiff=diff;
                    ret=(float)Math.sqrt(_r2);
                }
                //test[i][j]=distortion.distort(_r2);
            }
        }
        return ret;
    }
