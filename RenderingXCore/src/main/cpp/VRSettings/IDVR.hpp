#ifndef CONSTI_10_100_IDVR
#define CONSTI_10_100_IDVR

class IDVR{
public:
    static constexpr const char* GroundHeadTrackingEnable="GroundHeadTrackingEnable";
    static constexpr const char* GroundHeadTrackingX="GroundHeadTrackingX";
    static constexpr const char* GroundHeadTrackingY="GroundHeadTrackingY";
    static constexpr const char* GroundHeadTrackingZ="GroundHeadTrackingZ";
    static constexpr const char* AirHeadTrackingMode="AirHeadTrackingMode";
    static constexpr const char* AirHeadTrackingYaw="AirHeadTrackingYaw";
    static constexpr const char* AirHeadTrackingRoll="AirHeadTrackingRoll";
    static constexpr const char* AirHeadTrackingPitch="AirHeadTrackingPitch";
    static constexpr const char* AHTRefreshRateMs="AHTRefreshRateMs";
    static constexpr const char* GHT_OSD_FIXED_TO_HEAD="GHT_OSD_FIXED_TO_HEAD";
    static constexpr const char* VR_INVERT_LEFT_AND_RIGHT="VR_INVERT_LEFT_AND_RIGHT";
    static constexpr const char* SuperSync="SuperSync";
    static constexpr const char* DisableVSYNC="DisableVSYNC";
    static constexpr const char* Disable60FPSLock="Disable60FPSLock";
    static constexpr const char* MultiSampleAntiAliasing="MultiSampleAntiAliasing";
    static constexpr const char* ENABLE_LOW_PERSISTENCE="Enable low persistence mode";
    static constexpr const char* VR_SCENE_SCALE_PERCENTAGE="VR_SCENE_SCALE_PERCENTAGE";
    static constexpr const char* VR_DISTORTION_CORRECTION_MODE="VR_DISTORTION_CORRECTION_MODE";
    static constexpr const char* VR_ENABLE_DEBUG="VR_ENABLE_DEBUG";
    static constexpr const char* VR_SCREEN_BRIGHTNESS_PERCENTAGE="VR_SCREEN_BRIGHTNESS_PERCENTAGE";
    static constexpr const char* VR_RENDERING_MODE="VR_RENDERING_MODE";
};

#endif //CONSTI_10_100_IDVR
